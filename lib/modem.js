...
console.log("cmgr:" + cmgr);
            if (cmgr === null || cmgr === undefined || cmgr === "")
                return;
            var lines = cmgr.trim().split("\n");
    ...
    modem.getPhoneNumber = function(callback) {
        this.execute('AT+CNUM', function(data) {

            var lines = data.replace("\n", ",");
            var tmp = lines.match(/(\+)?\d{7,}/i);

            var number = tmp != null ? tmp[0] : null;

            if(callback)
                callback(number);
        }.bind(this), false, 3000);
    }

    modem.setPhoneNumber = function(number, callback) {
        this.execute('AT+CPBS="ON"', false, 3000);
        this.execute('AT+CPBW=1,"' + number  + '",145', false, 3000);
        this.execute('AT+CPBR=1', false, 3000);
        this.getPhoneNumber(function(number) {
            if(callback)
                callback(number);
        });
    }
